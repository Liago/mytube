...
exports.handler = async (event, context) => {
	...
	if (!videoId) {
		logger.error("Missing videoId parameter");
		await logger.save();
		return {
			statusCode: 400,
	...
	logger.info(`Starting upload/prefetch for ${videoId}`);
	...
	try {
		...
		try {
			...
			if (head.ContentLength && head.ContentLength > 0) {
				...
				logger.warn(`Cache HIT for ${videoId} but file is empty (0 bytes). Re-downloading...`);
			}
		} catch (error) {
			if (error.name !== "NotFound") {
				logger.error(`Error checking cache: ${error.message}`);
			}
			logger.info(`Cache MISS for ${videoId}. Downloading with local yt-dlp binary...`);
		}
		...
		try {
			...
			try {
				logger.info('Loading cookies from R2 bucket...');
				...
				logger.info(`Cookies loaded from R2 (${cookies.length} cookies), saved to ${netscapeCookiePath}`);
				activeCookiePath = netscapeCookiePath;
				hasCookies = true;
			} catch (err) {
				if (err.name === 'NoSuchKey' || err.name === 'NotFound') {
					logger.info('No _cookies.json found in R2 bucket');
				} else {
					logger.warn(`Error loading cookies from R2: ${err.message}`);
				}
			}
			...
			if (!hasCookies) {
				...
				if (fs.existsSync(jsonCookiePath)) {
					logger.info('Found local cookies.json, converting to Netscape format...');
					try {
						const jsonContent = fs.readFileSync(jsonCookiePath, 'utf8');
						const cookies = JSON.parse(jsonContent);
						const netscapeContent = convertToNetscape(cookies);
						fs.writeFileSync(netscapeCookiePath, netscapeContent);
						logger.info(`Converted cookies saved to ${netscapeCookiePath}`);
						activeCookiePath = netscapeCookiePath;
						hasCookies = true;
					} catch (err) {
						logger.warn(`Error converting cookies: ${err.message}`);
					}
				}
			}
			...
			logger.info(`OS: ${process.platform}, using binary: ${binaryName}`);

			if (!fs.existsSync(binaryPath)) {
				// Detailed error for debugging deployment
				const files = fs.readdirSync(process.cwd());
				logger.error(`Binary not found at ${binaryPath}. Root files: ${files.join(', ')}`);
			...
			const runYtDlp = async (useCookies, playerClient, ctx = { skipProxy: false }) => {
				...
				if (proxyUrl && !ctx.skipProxy && proxyUrl.startsWith('http')) {
					...
					logger.info('Proxy disabled for this request due to previous error.');
				} else if (proxyUrl) {
					logger.warn(`Ignoring invalid/placeholder PROXY_URL: ${proxyUrl}`);
				}

				const strategyLabel = `client=${playerClient}, cookies=${useCookies}`;
				logger.info(`Spawning (${strategyLabel}): ${binaryPath} ${args.join(' ')}`);
			...
			if (fs.existsSync(possibleMetaPath)) {
				logger.info(`Found metadata at ${possibleMetaPath}, uploading...`);
				...
				logger.info(`Metadata file not found at ${possibleMetaPath}`);
			}
			...
			try {
				...
				logger.warn(`Failed to cleanup tmp file: ${e.message}`);
			}
			...
			logger.info(`Upload complete for ${videoId}`);
		...
		logger.error(`Fatal function error: ${globalError.message}`);
		return {
			statusCode: 500,
